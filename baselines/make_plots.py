...
Usage:
    ...
    python baselines/make_plots.py --filter-type binary_fuse
"""
...
def print_table_markdown(title, experiments, value_fn, fmt=".1f"):
    """Print table in markdown format and return as string."""
    dists = sorted(set(exp["dataset"]["minority_dist"] for exp in experiments))
    grid = build_grid(experiments)
    lines = []
    ...
    for dist in dists:
        ...
        for n in ns:
            ...
            for method in methods:
                row = f"| {METHOD_DISPLAY.get(method, method)} |"
                for a in alphas:
                    exp = grid.get((dist, n, a))
                    if exp is None:
                        row += " — |"
...
def main():
    ...
    md_sections = []
    md_sections.append(f"# Baseline Results — {args.filter_type}\n")
    md_sections.append(
